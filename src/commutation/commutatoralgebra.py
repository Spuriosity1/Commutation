...
class CommutatorUnknownException(Exception):
    def __init__(self, o1, o2):
        super().__init__(f"commutator algebra does not know [{o1}, {o2}]_- or [{o1}, {o2}]_+")
...
#                     extra_terms += front * c * back
                    ...
                    if default_to_commutator:
                        c = self.get_commutator(term.ops[i], term.ops[i+1])
                        if c is not None:
                            extra_terms += front * c * back * term.multiplier
                        else:
                            # fall back to using anticommutator
                ...
                for i in range(I, 0, -1):
                    ...
                    if default_to_commutator:
                        ...
                        if c is not None:
                            ...
                            # fallback to anticommutator
                            c = self.get_anticommutator(
                                term.ops[i-1], term.ops[i])
                            if c is None:
                                raise CommutatorUnknownException(term.ops[i-1], term.ops[i])
                            extra_terms += Expression(front) * \
                                c * Expression(back)*term.multiplier
                            term.multiplier *= -1
                    else:
